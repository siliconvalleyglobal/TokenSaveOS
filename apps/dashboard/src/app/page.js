...
function Dashboard() {
    ...
    const [promptInput, setPromptInput] = (0, react_1.useState)("Can you please analyze this code and explain what improvements can be made?");
    const [simulatedResult, setSimulatedResult] = (0, react_1.useState)(null);
    ...
    return (<div className="min-h-screen p-6 md:p-10 space-y-8">
      {/* Executive Navbar */}
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between glass-card p-6 rounded-2xl gap-4">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-indigo-600/20 border border-indigo-500/30 rounded-xl text-indigo-400">
            <lucide_react_1.Zap className="w-7 h-7"/>
          </div>
          <div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchStats} className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-sm font-medium rounded-xl transition">
            <lucide_react_1.RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`}/> Refresh
          </button>
          <a href="https://svg.ph" target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-xl shadow-lg shadow-indigo-600/30 transition">
            svg.ph <lucide_react_1.ArrowUpRight className="w-4 h-4"/>
          </a>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="glass-card glass-card-hover p-6 rounded-2xl space-y-3">
          <div className="flex items-center justify-between text-indigo-400">
            ...
            <div className="p-2 bg-indigo-500/10 rounded-lg"><lucide_react_1.Zap className="w-5 h-5"/></div>
          </div>
          <div className="text-3xl font-extrabold text-white">
            {stats ? (stats.totalTokensSaved || 0).toLocaleString() : '1,482,900'}
          </div>
          <div className="text-xs text-emerald-400 flex items-center gap-1 font-medium">
            <lucide_react_1.TrendingDown className="w-3.5 h-3.5"/> 88% Net Payload Reduction
          </div>
        </div>
        ...
        <div className="glass-card glass-card-hover p-6 rounded-2xl space-y-3">
          <div className="flex items-center justify-between text-emerald-400">
            ...
            <div className="p-2 bg-emerald-500/10 rounded-lg"><lucide_react_1.DollarSign className="w-5 h-5"/></div>
          </div>
          <div className="text-3xl font-extrabold text-white">
            ${stats ? (stats.totalCostSavedUSD || 0).toFixed(2) : '7,414.50'}
          </div>
          <div className="text-xs text-emerald-400 flex items-center gap-1 font-medium">
            <lucide_react_1.CheckCircle2 className="w-3.5 h-3.5"/> Direct Monthly Savings
          </div>
        </div>
        ...
        <div className="glass-card glass-card-hover p-6 rounded-2xl space-y-3">
          <div className="flex items-center justify-between text-purple-400">
            ...
            <div className="p-2 bg-purple-500/10 rounded-lg"><lucide_react_1.BarChart3 className="w-5 h-5"/></div>
          </div>
          <div className="text-3xl font-extrabold text-white">
        ...
        <div className="glass-card glass-card-hover p-6 rounded-2xl space-y-3">
          <div className="flex items-center justify-between text-amber-400">
            ...
            <div className="p-2 bg-amber-500/10 rounded-lg"><lucide_react_1.Layers className="w-5 h-5"/></div>
          </div>
          <div className="text-3xl font-extrabold text-white">
        ...
        <div className="glass-card p-6 rounded-2xl lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <lucide_react_1.Activity className="w-5 h-5 text-indigo-400"/> Token Volume Before vs After Compression
              </h2>
          ...
          <div className="h-72 w-full">
            <recharts_1.ResponsiveContainer width="100%" height="100%">
              <recharts_1.AreaChart data={mockChartData}>
                <defs>
                ...
                <recharts_1.XAxis dataKey="name" stroke="#64748b" fontSize={12}/>
                <recharts_1.YAxis stroke="#64748b" fontSize={12}/>
                <recharts_1.Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px' }}/>
                <recharts_1.Area type="monotone" dataKey="original" stroke="#6366f1" fillOpacity={1} fill="url(#colorOriginal)"/>
                <recharts_1.Area type="monotone" dataKey="compressed" stroke="#10b981" fillOpacity={1} fill="url(#colorCompressed)"/>
        ...
        <div className="glass-card p-6 rounded-2xl space-y-6 flex flex-col justify-between">
          <div>
            <h2 className="text-lg font-bold text-white flex items-center gap-2 mb-1">
              <lucide_react_1.Terminal className="w-5 h-5 text-indigo-400"/> Live Token Optimization Playground
            </h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea value={promptInput} onChange={(e) => setPromptInput(e.target.value)} rows={4} className="w-full bg-slate-950/80 border border-slate-800 rounded-xl p-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition resize-none"/>
              </div>

              {simulatedResult && (<div className="space-y-3 bg-slate-950/50 p-4 rounded-xl border border-slate-800">
          ...
          <div className="p-3 bg-indigo-950/40 border border-indigo-800/40 rounded-xl text-xs text-indigo-300 flex items-center gap-2">
            <lucide_react_1.ShieldCheck className="w-4 h-4 flex-shrink-0 text-indigo-400"/>
